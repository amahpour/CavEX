...
static void server_local_process(struct server_rpc* call, void* user) {
	...
	switch(call->type) {
			...
			if(s->player.has_pos && call->payload.block_dig.y >= 0
			   ...
			   && call->payload.block_dig.finished) {
				...
				if(server_world_get_block(&s->world, call->payload.block_dig.x,
					...
					// "no item consumption"); survival drop rules are unchanged.
					if(!s->player.creative && blocks[blk.type]
					   && ((it
		...
		case SRPC_SET_GAMEMODE:
			// Server-authoritative creative toggle. Flip on request, then echo
			// the resulting state so the client mirror (HUD + dig feel) tracks
			// it. Ignored until a world is loaded.
			if(s->player.has_pos) {
				if(call->payload.set_gamemode.toggle)
					s->player.creative = !s->player.creative;

				clin_rpc_send(&(struct client_rpc) {
					.type = CRPC_GAMEMODE,
					.payload.gamemode.creative = s->player.creative,
				});
			}
			break;
		...
		case SRPC_CREATIVE_SET_PICKED: {
			// Survival-style creative grid (issue: creative move/select): put a
			// full, non-depleting stack of the requested item onto the cursor
			// (picked item), or clear the cursor when id == 0. The grid is a
			// virtual, infinite source, so this never touches a real slot -- the
			// normal SRPC_WINDOW_CLICK path then moves the cursor stack into the
			// player's hotbar/inventory exactly like survival. Any item id is
			...
			if(s->player.has_pos && s->player.creative) {
				...
				struct item_data picked
					= {.id = 0, .durability = 0, .count = 0};
				if(id > 0 && id < ITEMS_MAX && items[id]
				   && items[id]->renderItem) {
					picked.id = id;
		...
		case SRPC_BOAT_CONTROL: {
			...
			if(e && (e->type == ENTITY_BOAT || e->type == ENTITY_MINECART)) {
		...
		case SRPC_UNLOAD_WORLD:
			...
			// gen_world.py; legacy survival saves without the tag stay survival).
			level_archive_write(&s->level, LEVEL_PLAYER_GAMEMODE,
		...
		case SRPC_LOAD_WORLD:
			...
			if(level_archive_create(&s->level, s->level_name)) {
				...
				// Read the persisted game mode; a missing tag means survival.
				// (1 = creative in Beta 1.7.3.)
				int8_t game_mode = 0;
				level_archive_read(&s->level, LEVEL_PLAYER_GAMEMODE, &game_mode,
								   0);
				s->player.creative = (game_mode == 1);
...
static void server_local_update(struct server_local* s) {
	...
	if(c_nearest
	   && server_world_load_chunk(&s->world, c_nearest_x, c_nearest_z, &sc)) {
		...
		// Mirror the loaded game mode to the client so the HUD + dig-timer feel
		// are correct from the first frame (e.g. spawning into a creative world).
		clin_rpc_send(&(struct client_rpc) {
			.type = CRPC_GAMEMODE,
			.payload.gamemode.creative = s->player.creative,
		});
